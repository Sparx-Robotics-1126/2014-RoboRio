...
public class Drives extends GenericSubsystem {
	 ...
	 */
	private AnalogInput gyroAnalog;
	 ...
	 */
	public Drives(){
		super("Drives", Thread.NORM_PRIORITY);//This may be changed to max 
		wantedLeftSpeed = 0;
		wantedRightSpeed = 0;
	}
	 ...
	 */
	public boolean init() {
		leftFrontDrives = new Talon(IO.LEFT_FRONT_DRIVES_PWM);
		leftRearDrives = new Talon(IO.LEFT_REAR_DRIVES_PWM);
		leftBottomDrives = new Talon(IO.LEFT_BOTTOM_DRIVES_PWM);
		leftDrivesEncoder = new Encoder(IO.LEFT_DRIVES_ENCODER_CHAN_1, IO.LEFT_DRIVES_ENCODER_CHAN_2, false, EncodingType.k4X);
		leftDrivesEncoder.setDistancePerPulse(DIST_PER_TICK);
		leftEncoderData = new EncoderData(leftDrivesEncoder, DIST_PER_TICK);

		rightFrontDrives = new Talon(IO.RIGHT_FRONT_DRIVES_PWM);
		rightRearDrives = new Talon(IO.RIGHT_REAR_DRIVES_PWM);
		rightBottomDrives = new Talon(IO.RIGHT_BOTTOM_DRIVES_PWM);
		rightDrivesEncoder = new Encoder(IO.RIGHT_DRIVES_ENCODER_CHAN_1, IO.RIGHT_DRIVES_ENCODER_CHAN_2, true, EncodingType.k4X);
		...
		gyroAnalog = new AnalogInput(IO.GYRO_ANALOG);
